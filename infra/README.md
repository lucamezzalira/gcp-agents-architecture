...
| --- | --- | --- |
| Agent Runtime | `google_vertex_ai_reasoning_engine.agent` with `spec.container_spec` | Live engine in `europe-west1`. After `agent_score_split` it uses `agent_reasoner_image` (no scoring, no Postgres). Until then it keeps the scoring image so the dashboard stays live. `container_spec.image_uri` rejects digest pins; Terraform uses `split("@", image)[0]`. Memory Bank stays in `us-central1`. |
| Reasoner preview | `google_vertex_ai_reasoning_engine.reasoner_preview` | Created when `agent_reasoner_image` is set and `agent_score_split` is false. Same Identity and Gemini Pro as the live engine. Destroyed on cutover. |
...
`agent_score_split` is the live-traffic cutover, not the architecture. While it is false, Pub/Sub still hits a Cloud Run doorway whose engine image still contains scoring, and a preview reasoner stands beside it. Set it true so Cloud Run takes scoring and Postgres, the live engine switches to the reasoner image, and the preview engine is destroyed.
