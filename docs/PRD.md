...
- **Scoring and reasoning split.** The flow below has the health agent subscribed to the analysis topic, loading accepted decisions, and producing the health read. The shipped system puts a Cloud Run receiver on that subscription. The receiver validates the payload, scores it with `health/scoring`, persists the run to Postgres, acks that push, and enqueues Agent Runtime on `analysis-reason`. The agent receives scores already fixed, reasons, reads and writes Memory Bank, and returns prose. The receiver attaches that prose to the existing rows.
